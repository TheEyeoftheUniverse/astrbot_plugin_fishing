...
class GameMechanicsService:
    ...
    def handle_wof_timeout(self, user_id: str) -> Dict[str, Any] | None:
        ...
        # 转换为 naive datetime 以避免时区比较问题
        last_action_time = user.wof_last_action_time
        ...
        if (now - last_action_time).total_seconds() > timeout_seconds:
    ...
    def start_wheel_of_fate(self, user_id: str, entry_fee: int) -> Dict[str, Any]:
        ...
        # 转换为 naive datetime 以避免时区比较问题
        if user.last_wof_play_time:
            last_play_time = user.last_wof_play_time
            if hasattr(last_play_time, 'tzinfo') and last_play_time.tzinfo is not None:
                last_play_time = last_play_time.replace(tzinfo=None)
            
            if (now - last_play_time).total_seconds() < cooldown:
                remaining = int(cooldown - (now - last_play_time).total_seconds())
    ...
    def play_sicbo(self, user_id: str, bet_type: str, amount: int) -> Dict[str, Any]:
        ...
        # 转换为 naive datetime 以避免时区比较问题
        if user.last_sicbo_time:
            last_sicbo_time = user.last_sicbo_time
            if hasattr(last_sicbo_time, 'tzinfo') and last_sicbo_time.tzinfo is not None:
                last_sicbo_time = last_sicbo_time.replace(tzinfo=None)
            
            if (now - last_sicbo_time).total_seconds() < cooldown_seconds:
                remaining = int(cooldown_seconds - (now - last_sicbo_time).total_seconds())
